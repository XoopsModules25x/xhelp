...
class Legend
{
    ...
    public function SetHColMargin($aXMarg): void
    {
        $this->xmargin = $aXMarg;
    }

    public function SetVColMargin($aSpacing): void
    {
        $this->ylinespacing = $aSpacing;
    }

    public function SetLeftMargin($aXMarg): void
    {
        $this->xlmargin = $aXMarg;
    }

    // Synonym
    public function SetLineSpacing($aSpacing): void
    {
        $this->ylinespacing = $aSpacing;
    }

    public function SetShadow($aShow = 'gray', $aWidth = 4): void
    {
        if (is_string($aShow)) {
    ...
    public function SetMarkAbsSize($aSize): void
    {
        $this->mark_abs_vsize = $aSize;
        $this->mark_abs_hsize = $aSize;
    }

    public function SetMarkAbsVSize($aSize): void
    {
        $this->mark_abs_vsize = $aSize;
    }

    public function SetMarkAbsHSize($aSize): void
    {
        $this->mark_abs_hsize = $aSize;
    }
    ...
    public function SetLayout($aDirection = LEGEND_VERT): void
    {
        $this->layout_n = LEGEND_VERT == $aDirection ? 1 : 99;
    }

    public function SetColumns($aCols): void
    {
        $this->layout_n = $aCols;
    }

    public function SetReverse($f = true): void
    {
        $this->reverse = $f;
    }

    // Set color on frame around box
    public function SetColor($aFontColor, $aColor = 'black'): void
    {
    ...
    public function SetPos($aX, $aY, $aHAlign = 'right', $aVAlign = 'top'): void
    {
        $this->Pos($aX, $aY, $aHAlign, $aVAlign);
    }

    public function SetAbsPos($aX, $aY, $aHAlign = 'right', $aVAlign = 'top'): void
    {
        $this->xabspos = $aX;
        $this->yabspos = $aY;
        $this->halign  = $aHAlign;
    ...
    public function Pos($aX, $aY, $aHAlign = 'right', $aVAlign = 'top'): void
    {
        if (!($aX < 1 && $aY < 1)) {
    ...
    public function Add($aTxt, $aColor, $aPlotmark = '', $aLinestyle = 0, $csimtarget = '', $csimalt = '', $csimwintarget = ''): void
    {
    ...
    public function SetBackgroundGradient($aFrom = 'navy', $aTo = 'silver', $aGradType = 2): void
    {
        ...
        foreach ($this->txtcol as $p) {
            ...
            if (!empty($p[2]) && $p[2]->GetType() > -1) {
                ...
                if ($p[3] < -100) {
                    ...
                    if (is_array($color) && 2 == count($color)) {
            ...
            if (!empty($p[4])) {
                ...
                if (!empty($p[4])) {
                    $this->csimareas .= "<area shape=\"poly\" coords=\"$coords\" href=\"" . htmlentities($p[4], ENT_QUOTES | ENT_HTML5) . '"';
