...
class Text
{
    ...
    // Alias for Pos()
    public function SetPos($aXAbsPos = 0, $aYAbsPos = 0, $aHAlign = 'left', $aVAlign = 'top'): void
    {
    ...
    public function Align($aHAlign, $aVAlign = 'top', $aParagraphAlign = ''): void
    {
        $this->halign = $aHAlign;
        $this->valign = $aVAlign;
        if ('' != $aParagraphAlign) {
    ...
    // Alias
    public function SetAlign($aHAlign, $aVAlign = 'top', $aParagraphAlign = ''): void
    {
        $this->Align($aHAlign, $aVAlign, $aParagraphAlign);
    }

    // Specifies the alignment for a multi line text
    public function ParagraphAlign($aAlign): void
    {
        $this->paragraph_align = $aAlign;
    }

    // Specifies the alignment for a multi line text
    public function SetParagraphAlign($aAlign): void
    {
        $this->paragraph_align = $aAlign;
    }

    public function SetShadow($aShadowColor = 'gray', $aShadowWidth = 3): void
    {
    ...
    public function SetWordWrap($aCol): void
    {
        $this->iWordwrap = $aCol;
    }
    ...
    public function SetBox($aFrameColor = [255, 255, 255], $aBorderColor = [0, 0, 0], $aShadowColor = false, $aCornerRadius = 4, $aShadowWidth = 3): void
    {
    ...
    public function SetBox2($aFrameColor = [255, 255, 255], $aBorderColor = [0, 0, 0], $aShadowColor = false, $aCornerRadius = 4, $aShadowWidth = 3): void
    {
        $this->iBoxType = 2;
    ...
    public function Center($aLeft, $aRight, $aYAbsPos = false): void
    {
    ...
    public function SetOrientation($aDirection = 0): void
    {
    ...
    // on the context.
    public function SetMargin($aMarg): void
    {
        $this->margin = $aMarg;
    }
    ...
    public function StrokeWithScale($aImg, $axscale, $ayscale): void
    {
        if (null === $this->iScalePosX || null === $this->iScalePosY) {
            ...
            $this->Stroke($aImg, round($axscale->Translate($this->iScalePosX)), round($ayscale->Translate($this->iScalePosY)));
        }
    }

    public function SetCSIMTarget($aURITarget, $aAlt = '', $aWinTarget = ''): void
    {
    ...
    public function Stroke($aImg, $x = null, $y = null): void
    {
        if (null !== $x) {
            $this->x = round($x);
        }
        ...
        if ($this->boxed) {
            ...
            if (2 == $this->iBoxType && $this->font_family > FF_FONT2 + 2) {
        ...
        $this->iCSIMarea = "<area shape=\"poly\" coords=\"$coords\" href=\"" . htmlentities($this->iCSIMtarget, ENT_QUOTES | ENT_HTML5) . '" ';
