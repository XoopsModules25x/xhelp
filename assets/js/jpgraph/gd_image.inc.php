...
require_once __DIR__ . '/imageSmoothArc.php';
...
class Image
{
    ...
    public function SetInterlace($aFlg = true): void
    {
        $this->iInterlace = $aFlg;
    }

    // Should we use anti-aliasing. Note: This really slows down graphics!
    public function SetAntiAliasing($aFlg = true): void
    {
    ...
    public function CreateRawCanvas($aWidth = 0, $aHeight = 0): void
    {
        ...
        if ($this->img < 1) {
            ...
            //exit("Can't create truecolor image. Check that you really have GD2 library installed.");
        }
    ...
    public function CopyCanvasH($aToHdl, $aFromHdl, $aToX, $aToY, $aFromX, $aFromY, $aWidth, $aHeight, $aw = -1, $ah = -1): void
    {
        if (-1 === $aw) {
            $aw = $aWidth;
            $ah = $aHeight;
    ...
    public function Copy($fromImg, $toX, $toY, $fromX, $fromY, $toWidth, $toHeight, $fromWidth = -1, $fromHeight = -1): void
    {
        $this->CopyCanvasH($this->img, $fromImg, $toX, $toY, $fromX, $fromY, $toWidth, $toHeight, $fromWidth, $fromHeight);
    }

    public function CopyMerge($fromImg, $toX, $toY, $fromX, $fromY, $toWidth, $toHeight, $fromWidth = -1, $fromHeight = -1, $aMix = 100): void
    {
        if (100 == $aMix) {
            $this->CopyCanvasH($this->img, $fromImg, $toX, $toY, $fromX, $fromY, $toWidth, $toHeight, $fromWidth, $fromHeight);
        } else {
    ...
    public function SetCanvasH($aHdl): void
    {
        $this->img      = $aHdl;
        $this->rgb->img = $aHdl;
    }

    public function SetCanvasColor($aColor): void
    {
    ...
    public function SetAutoMargin(): void
    {
        $min_bm = 5;
    ...
    public function SetFont($family, $style = FS_NORMAL, $size = 10): void
    {
    ...
    public function StrokeBoxedText2(
        ...
        if ($shadowcolor) {
            $this->PushColor($shadowcolor);
            $this->FilledRectangle($x - $xmarg + $dropwidth, $y + $ymarg + $dropwidth - $height, $x + $width + $dropwidth, $y + $ymarg + $dropwidth);
            //$cornerradius);
            $this->PopColor();
            $this->PushColor($fcolor);
            $this->FilledRectangle($x - $xmarg, $y + $ymarg - $height, $x + $width, $y + $ymarg);
            //$cornerradius);
            $this->PopColor();
            $this->PushColor($bcolor);
            $this->Rectangle($x - $xmarg, $y + $ymarg - $height, $x + $width, $y + $ymarg);
            //$cornerradius);
            $this->PopColor();
        } else {
            if ($fcolor) {
    ...
    // Set text alignment
    public function SetTextAlign($halign, $valign = 'bottom'): void
    {
        $this->text_halign = $halign;
        $this->text_valign = $valign;
    }

    public function _StrokeBuiltinFont($x, $y, $txt, $dir, $paragraph_align, &$aBoundingBox, $aDebug = false): void
    {
    ...
    public function _StrokeTTF($x, $y, $txt, $dir, $paragraph_align, &$aBoundingBox, $debug = false): void
    {
        ...
        if (!preg_match('/\n/', $txt) || ($dir > 0 && preg_match('/\n/', $txt))) {
            ...
            if ('basepoint' != $this->text_valign) {
                ...
                if ('right' == $this->text_halign) {
                    ...
                    $x -= $width / 2;
                    $x -= $bbox[0];
                } elseif ('baseline' == $this->text_halign) {
                ...
                if ('top' == $this->text_valign) {
                    ...
                    $y -= $bbox[1]; // Adjust to bottom of text
                    $y += $height / 2;
                } elseif ('baseline' == $this->text_valign) {
    ...
    public function SetTransparent($color): void
    {
    ...
    public function PushColor($color): void
    {
        if ('' != $color) {
    ...
    public function Arc($cx, $cy, $w, $h, $s, $e): void
    {
    ...
    public function FilledArc($xc, $yc, $w, $h, $s, $e, $style = ''): void
    {
        $s = round($s);
        $e = round($e);
        while ($s < 0) {
    ...
    public function FilledCakeSlice($cx, $cy, $w, $h, $s, $e): void
    {
        $this->CakeSlice($cx, $cy, $w, $h, $s, $e, $this->current_color_name);
    }

    public function CakeSlice($xc, $yc, $w, $h, $s, $e, $fillcolor = '', $arccolor = ''): void
    {
        $s  = round($s);
        $e  = round($e);
        $w  = round($w);
    ...
    public function Ellipse($xc, $yc, $w, $h): void
    {
        $this->Arc($xc, $yc, $w, $h, 0, 360);
    }

    public function Circle($xc, $yc, $r): void
    {
    ...
    public function FilledCircle($xc, $yc, $r): void
    {
    ...
    public function StyleLine($x1, $y1, $x2, $y2, $aStyle = '', $from_grid_class = false): void
    {
    ...
    public function DashedLine($x1, $y1, $x2, $y2, $dash_length = 1, $dash_space = 4): void
    {
        if ($this->line_weight <= 0) {
            return;
        }
    ...
    public function DashedLineForGrid($x1, $y1, $x2, $y2, $dash_length = 1, $dash_space = 4): void
    {
        if ($this->line_weight <= 0) {
            return;
        }
    ...
    public function Polygon($p, $closed = false, $fast = false): void
    {
        if ($this->line_weight <= 0) {
            return;
        }
    ...
    public function FilledPolygon($pts): void
    {
        $n = count($pts);
        if (0 == $n) {
    ...
    public function Rectangle($xl, $yu, $xr, $yl): void
    {
        $this->Polygon([$xl, $yu, $xr, $yu, $xr, $yl, $xl, $yl, $xl, $yu]);
    }

    public function FilledRectangle($xl, $yu, $xr, $yl): void
    {
        $this->FilledPolygon([$xl, $yu, $xr, $yu, $xr, $yl, $xl, $yl]);
    }

    public function FilledRectangle2($xl, $yu, $xr, $yl, $color1, $color2, $style = 1): void
    {
    ...
    public function ShadowRectangle($xl, $yu, $xr, $yl, $fcolor = false, $shadow_width = 4, $shadow_color = 'darkgray', $useAlpha = true): void
    {
        ...
        for ($i = 0; $i < $shadow_width; ++$i) {
            ...
            $this->Line($xr - $shadow_width + $i, $yu + $shadow_width, $xr - $shadow_width + $i, $yl - $shadow_width - 1 + $i);
            $this->Line($xl + $shadow_width, $yl - $shadow_width + $i, $xr - $shadow_width + $i, $yl - $shadow_width + $i);
            if ($useAlpha) {
    ...
    public function FilledRoundedRectangle($xt, $yt, $xr, $yl, $r = 5): void
    {
        if (0 == $r) {
    ...
    public function RoundedRectangle($xt, $yt, $xr, $yl, $r = 5): void
    {
        if (0 == $r) {
    ...
    public function FilledBevel($x1, $y1, $x2, $y2, $depth = 2, $color1 = 'white@0.4', $color2 = 'darkgray@0.4'): void
    {
        $this->FilledRectangle($x1, $y1, $x2, $y2);
        $this->Bevel($x1, $y1, $x2, $y2, $depth, $color1, $color2);
    }

    public function Bevel($x1, $y1, $x2, $y2, $depth = 2, $color1 = 'white@0.4', $color2 = 'black@0.5'): void
    {
    ...
    public function StyleLineTo($x, $y): void
    {
    ...
    public function SetExpired($aFlg = true): void
    {
        $this->expired = $aFlg;
    }
    ...
    public function SetQuality($q): void
    {
        $this->quality = $q;
    }

    // Stream image to browser or to file
    public function Stream($aFile = null): void
    {
    ...
    public function SetImgFormat($aFormat, $aQuality = 75)
    {
        ...
        if ('jpeg' == $aFormat || 'png' == $aFormat || 'gif' == $aFormat) {
            if ('jpeg' == $aFormat && !($supported & IMG_JPG)) {
                ...
                $tst = false;
            } elseif ('wbmp' == $aFormat && !($supported & IMG_WBMP)) {
                $tst = false;
            } elseif ('xpm' == $aFormat && !($supported & IMG_XPM)) {
                $tst = false;
            } else {
    ...
    public function imageSmoothCircle($img, $cx, $cy, $cr, $color): void
    {
        $ir   = $cr;
        $ix   = 0;
        $iy   = $ir;
...
class RotImage extends Image
{
    ...
    public function UpdateRotMatrice(): void
    {
        $a  = $this->a;
        $a  *= M_PI / 180;
        $sa = sin($a);
    ...
    public function Arc($xc, $yc, $w, $h, $s, $e): void
    {
        [$xc, $yc] = $this->Rotate($xc, $yc);
        $s += $this->a;
        $e += $this->a;
    ...
    public function Rectangle($x1, $y1, $x2, $y2): void
    {
        // Rectangle uses Line() so it will be rotated through that call
        parent::Rectangle($x1, $y1, $x2, $y2);
    }

    public function FilledRectangle($x1, $y1, $x2, $y2): void
    {
        if ($y1 == $y2 || $x1 == $x2) {
            $this->Line($x1, $y1, $x2, $y2);
        } else {
    ...
    public function Polygon($pnts, $closed = false, $fast = false): void
    {
    ...
    public function FilledPolygon($pnts): void
    {
...
class ImgStreamCache
{
    ...
    public function SetTimeout($aTimeout): void
    {
        $this->timeout = $aTimeout;
    }

    // Output image to browser and also write it to the cache
    public function PutAndStream($aImage, $aCacheFileName, $aInline, $aStrokeFileName): void
    {
