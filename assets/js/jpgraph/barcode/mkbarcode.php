                ...
                switch ($this->argv[$i]) {
                    ...
                    default:
                        if ('' == $data) {
                            ...
                            $this->PrintUsage();
                            exit('Illegal specified parameters');
                        }
                        break;
                }
                ++$i;
...
class Driver
{
    ...
    public static function ErrHandlerPS(\Throwable $e): void
    {
        if (!self::$silent) {
    ...
    public static function ErrHandlerImg(\Throwable $e): void
    {
        if (!self::$silent) {
            fwrite(STDERR, $e->getMessage() . "\n");
        }
    ...
    public function Run($aParams): void
    {
        $this->iParams = $aParams;

        self::$silent = $aParams['silent'];
        ...
        switch ($aParams['output']) {
            case 0:
                ...
                set_exception_handler(['Driver', 'ErrHandlerImg']);
                break;
            case 1:
                $e = BackendFactory::Create(BACKEND_PS, $encoder);
                set_exception_handler(['Driver', 'ErrHandlerPS']);
                break;
            case 2:
                $e = BackendFactory::Create(BACKEND_PS, $encoder);
                $e->SetEPS();
