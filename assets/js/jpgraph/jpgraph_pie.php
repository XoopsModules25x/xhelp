...
class PiePlot
{
    ...
    //---------------
    // PUBLIC METHODS
    public function SetCenter($x, $y = 0.5): void
    {
        $this->posx = $x;
        $this->posy = $y;
    }
    ...
    public function SetGuideLines($aFlg = true, $aCurved = true, $aAlways = false): void
    {
    ...
    public function SetGuideLinesAdjust($aVFactor, $aRFactor = 0.8): void
    {
    ...
    public function SetSliceColors($aColors): void
    {
        $this->setslicecolors = $aColors;
    }

    public function SetShadow($aColor = 'darkgray', $aDropWidth = 4): void
    {
    ...
    public function AddSliceToCSIM($i, $xc, $yc, $radius, $sa, $ea): void
    {
    ...
    public function SetTheme($aTheme): void
    {
    ...
    public function ExplodeSlice($e, $radius = 20): void
    {
        if (!is_int($e)) {
    ...
    public function ExplodeAll($radius = 20): void
    {
        $this->explode_all = true;
        $this->explode_r   = $radius;
    }

    public function Explode($aExplodeArr): void
    {
    ...
    public function SetStartAngle($aStart): void
    {
    ...
    // Set label arrays
    public function SetLegends($aLegend): void
    {
        $this->legends = $aLegend;
    }

    // Set text labels for slices
    public function SetLabels($aLabels, $aLblPosAdj = 'auto'): void
    {
        $this->labels       = array_reverse($aLabels);
        $this->ilabelposadj = $aLblPosAdj;
    }

    public function SetLabelPos($aLblPosAdj): void
    {
        $this->ilabelposadj = $aLblPosAdj;
    }

    // Should we display actual value or percentage?
    public function SetLabelType($aType): void
    {
        if ($aType < 0 || $aType > 2) {
    ...
    // Deprecated.
    public function SetValueType($aType): void
    {
        $this->SetLabelType($aType);
    }

    // Should the circle around a pie plot be displayed
    public function ShowBorder($exterior = true, $interior = true): void
    {
    ...
    // Setup the legends
    public function Legend($graph): void
    {
    ...
    public function Stroke($img, $aaoption = 0): void
    {
        ...
        if ($this->radius <= 1) {
            ...
            $radius = 1 === $aaoption ? $this->radius * 2 : $this->radius;
        }
        ...
        if ('' != $this->ishadowcolor && 2 !== $aaoption) {
            $accsum = 0;
        ...
        for ($i = 0; $sum > 0 && $i < $n; ++$i) {
            ...
            if ($this->pie_interior_border && 0 === $aaoption) {
                $img->SetColor($this->color);
            } else {
                $img->SetColor($slicecolor);
            }
            $arccolor = $this->pie_border && 0 === $aaoption ? $this->color : '';
            ...
            if ($this->csimtargets && 1 !== $aaoption) {
    ...
    public function StrokeGuideLabels($img, $xc, $yc, $radius): void
    {
        $n = count($this->labels);
            ...
            if ($diff < $tresh_hold) {
                if ($incluster) {
                    ...
                    if ($q1 != $q2) {
                        ...
                        // cluster
                        if (1 == $q1 && 0 == $q2 && $a2 > (90 - 15) * M_PI / 180) {
                            if ($i < $n - 2) {
                                $a3 = $this->la[$i + 2];
                        ...
                        } elseif (3 == $q1 && 2 == $q2 && $a2 > (270 - 15) * M_PI / 180) {
                            if ($i < $n - 2) {
                                $a3 = $this->la[$i + 2];
                                // If there isn't a cluster coming up with the next-next slice
                                // we extend the previous cluster to cover this slice as well
                        ...
                        if (2 == $q1 && 1 == $q2 && $a2 > (180 - 15) * M_PI / 180) {
                            $clusters[$cidx][1]++;
                            $i++;
                        }
                    ...
                    if (0 == $q1 && $cidx > -1
                        && 1 == $clusters[$cidx][1]
                        && 0 == $this->Quadrant($this->la[$clusters[$cidx][0]])) {
                        $clusters[$cidx][1]++;
                    } else {
                        $cidx++;
                        $clusters[$cidx][0] = $i;
    ...
    public function StrokeAllLabels($img, $xc, $yc, $radius): void
    {
    ...
    public function StrokeLabel($label, $img, $xc, $yc, $a, $r): void
    {
        // Default value
    ...
    public function UsePlotThemeColors($flag = true): void
    {
...
class PiePlotC extends PiePlot
{
    ...
    public function SetMid($aTitle, $aColor = 'white', $aSize = 0.5): void
    {
    ...
    public function SetMidTitle($aTitle): void
    {
        $this->midtitle->Set($aTitle);
    }

    public function SetMidSize($aSize): void
    {
        $this->imidsize = $aSize;
    }

    public function SetMidColor($aColor): void
    {
        $this->imidcolor = $aColor;
    }

    public function SetMidCSIM($aTarget, $aAlt = '', $aWinTarget = ''): void
    {
    ...
    public function AddMiddleCSIM($xc, $yc, $r): void
    {
        $xc              = round($xc);
        $yc              = round($yc);
        $r               = round($r);
...
class PieGraph extends Graph
{
    ...
    public function DisplayCSIMAreas(): void
    {
        $csim = '';
