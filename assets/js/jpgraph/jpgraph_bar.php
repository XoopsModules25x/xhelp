...
class BarPlot extends Plot
{
    ...
    public function SetShadow($aColor = 'black', $aHSize = 3, $aVSize = 3, $aShow = true): void
    {
    ...
    public function Set3D($aHSize = 3, $aVSize = 3, $aShow = true): void
    {
        $this->bar_3d       = $aShow;
    ...
    public function SetYMin($aYStartValue): void
    {
        //exit("JpGraph Error: Deprecated function SetYMin. Use SetYBase() instead.");
        $this->ybase = $aYStartValue;
    }

    // Specify the base value for the bars
    public function SetYBase($aYStartValue): void
    {
        $this->ybase = $aYStartValue;
    }
    ...
    // overrides SetWidth()
    public function SetAbsWidth($aWidth): void
    {
        $this->abswidth = $aWidth;
    }

    public function SetAlign($aAlign): void
    {
        $this->align = $aAlign;
    }
    ...
    public function SetFillGradient($aFromColor, $aToColor = null, $aStyle = null): void
    {
        $this->grad           = true;
    ...
    public function SetValuePos($aPos): void
    {
        $this->valuepos = $aPos;
    }

    public function SetPattern($aPattern, $aColor = 'black'): void
    {
    ...
    public function _SetPatternHelper($aPattern, &$aPatternValue, &$aDensity): void
    {
        switch ($aPattern) {
            case PATTERN_DIAG1:
                $aPatternValue = 1;
        ...
        for ($i = 0; $i < $numbars; ++$i) {
            ...
            */
            ...
            if (!empty($this->csimtargets[$i])) {
                ...
                $this->csimareas .= ' href="' . htmlentities($this->csimtargets[$i], ENT_QUOTES | ENT_HTML5) . '"';
        ...
        for ($i = 0; $i < $this->numpoints - 1; ++$i) {
            ...
            for ($j = 0; $j < $this->nbrplots; ++$j) {
                ...
                if ($this->plots[$j]->coords[0][$i] > 0) {
                    $yt    = $yscale->Translate($this->plots[$j]->coords[0][$i] + $accy);
                    $accyt = $yscale->Translate($accy);
                    if ('center' == $this->plots[$j]->valuepos) {
                        $y = $accyt - ($accyt - $yt) / 2;
                    } elseif ('bottom' == $this->plots[$j]->valuepos) {
                        $y = $accyt;
                    } else { // top or max
                        $y = $accyt - ($accyt - $yt);
                    }
                    ...
                    if ('center' == $this->plots[$j]->valuepos) {
                        ...
                        $this->plots[$j]->value->SetAlign('center', 0 == $j ? 'bottom' : 'top');
                        $this->plots[$j]->value->SetMargin(-2);
                    } else {
                        $this->plots[$j]->value->SetAlign('center', 'bottom');
                        $this->plots[$j]->value->SetMargin(-1);
