...
require_once __DIR__ . '/jpgraph_log.php';
...
class PolarPlot
{
    ...
    public function SetCSIMTargets($aTargets, $aAlts = null): void
    {
    ...
    public function SetLegend($aLegend, $aCSIM = '', $aCSIMAlt = ''): void
    {
        $this->legend           = $aLegend;
    ...
    public function Legend($aGraph): void
    {
        ...
        if ('' != $this->legend) {
            if ('' != $this->iFillColor) {
                ...
                $aGraph->legend->Add($this->legend, $color, $this->mark, 0, $this->legendcsimtarget, $this->legendcsimalt);
            } else {
                $aGraph->legend->Add($this->legend, $color, $this->mark, $this->line_style, $this->legendcsimtarget, $this->legendcsimalt);
            }
        }
    }

    public function Stroke($img, $scale): void
    {
        $i               = 0;
        $p               = [];
        $this->csimareas = '';
...
class PolarAxis extends Axis
{
    ...
    public function ShowAngleDegreeMark($aFlg = true): void
    {
        $this->show_angle_mark = $aFlg;
    }

    public function SetAngleStep($aStep): void
    {
        $this->angle_step = $aStep;
    }

    public function HideTicks($aFlg = true, $aAngleFlg = true): void
    {
        parent::hideTicks($aFlg, $aFlg);
        $this->show_angle_tick = !$aAngleFlg;
    }

    public function ShowAngleLabel($aFlg = true): void
    {
        $this->show_angle_label = $aFlg;
    }

    public function ShowGrid($aMajor = true, $aMinor = false, $aAngle = true): void
    {
    ...
    public function SetAngleFont($aFontFam, $aFontStyle = FS_NORMAL, $aFontSize = 10): void
    {
    ...
    public function SetColor($aColor, $aRadColor = '', $aAngleColor = ''): void
    {
        if ('' == $aAngleColor) {
            $aAngleColor = $aColor;
        }
    ...
    public function SetGridColor($aMajorColor, $aMinorColor = '', $aAngleColor = ''): void
    {
        if ('' == $aMinorColor) {
    ...
    public function SetTickColors($aRadColor, $aAngleColor = ''): void
    {
    ...
    // Private methods
    public function StrokeGrid($pos): void
    {
        ...
        if ($this->show_angle_grid) {
            ...
            while ($a < 360) {
                if (90 == $a || 270 == $a) {
                    ...
                    $this->img->Line($x + $start_radius * cos($a / 180 * M_PI) + 1, $pos - $start_radius * sin($a / 180 * M_PI), $x + $start_radius * cos($a / 180 * M_PI) + 1, $pos - $d * sin($a / 180 * M_PI));
                } else {
                    $this->img->Line($x + $start_radius * cos($a / 180 * M_PI) + 1, $pos - $start_radius * sin($a / 180 * M_PI), $x + $d * cos($a / 180 * M_PI), $pos - $d * sin($a / 180 * M_PI));
                }
                $a += $this->angle_step;
            }
        }
    ...
    public function StrokeAngleLabels($pos, $type): void
    {
    ...
    public function Stroke($pos, $dummy = true): void
    {
        ...
        if ('high' == $this->title_adjust) {
            ...
            $this->title->SetPos(($this->img->width - $this->img->left_margin - $this->img->right_margin) / 2 + $this->img->left_margin, $y, 'center', 'top');
        ...
        $i   = 0;
        while ($i < $n) {
            $this->scale->ticks->ticks_pos[$i] = $mid - $this->scale->ticks->ticks_pos[$i];
            ++$i;
        }
        ...
        $i = 0;
        while ($i < $n) {
            $this->scale->ticks->maj_ticks_pos[$i] = $mid - $this->scale->ticks->maj_ticks_pos[$i];
            ++$i;
        }
        ...
        $i = 1;
        while ($i < $n) {
            $this->scale->ticks->maj_ticklabels_pos[$i] = $mid - $this->scale->ticks->maj_ticklabels_pos[$i];
            ++$i;
        }
...
class PolarScale extends LinearScale
{
    ...
    public function SetClockwise($aFlg): void
    {
        $this->clockwise = $aFlg;
    }
...
class PolarGraph extends Graph
{
    ...
    public function SetDensity($aDense): void
    {
    ...
    public function setScale($aScale, $rmax = 0, $dummy1 = 1, $dummy2 = 1, $dummy3 = 1): void
    {
        if ('lin' == $aScale) {
    ...
    public function SetPlotSize($w, $h): void
    {
        $this->setMargin(($this->img->width - $w) / 2, ($this->img->width - $w) / 2, ($this->img->height - $h) / 2, ($this->img->height - $h) / 2);
    }
    ...
    public function Stroke($aStrokeFileName = '')
    {
        ...
        if (!$this->scale->IsSpecified() && count($this->plots) > 0) {
            ...
            $this->scale->AutoScale($this->img, 0, $max, $this->img->plotwidth / $this->xtick_factor / 2);
        ...
        if ($this->iDoClipping) {
            ...
            if (0 == $this->img->a) {
                $this->img->CopyCanvasH($oldimage, $this->img->img, $this->img->left_margin, $this->img->top_margin, $this->img->left_margin, $this->img->top_margin, $this->img->plotwidth + 1, $this->img->plotheight + 1);
                ...
                $this->img->CopyCanvasH($oldimage, $this->img->img, $adj2 + round(($lm - $rm + $tm + $bm) / 2), $adj1 + round(($tm - $bm + $lm + $rm) / 2), $adj2 + round(($lm - $rm + $tm + $bm) / 2), $adj1 + round(($tm - $bm + $lm + $rm) / 2), $this->img->plotheight + 1, $this->img->plotwidth + 1);
            }
            $this->img->Destroy();
