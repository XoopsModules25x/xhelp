...
require_once __DIR__ . '/jpgraph/jpgraph_canvtools.php';
...
$tt->Set('Axis, Labelling, (Auto)-Scaling', $l, $r, $width * 0.9, $h - 2);
$tt->Stroke($g->img, $scale);

--$r;
$shape->IndentedRectangle($l, $r, $width, 7, $width * 0.9, 6, 3, 'goldenrod1');
$tt->Set('Error handling & Utility classes', $l, $r, $width, 1);
$tt->Stroke($g->img, $scale);
...
$t->Set("Error\nPlot", $l + 4 * $w, $r, $wp, $h);
$t->Stroke($g->img, $scale);
$t->Set("Line\nPlot", $l + 4 * $w + $wp, $r, $wp, $h);
$t->Stroke($g->img, $scale);
$t->Set("Bar\nPlot", $l + 4 * $w + 2 * $wp, $r, $wp, $h);
$t->Stroke($g->img, $scale);
$t->Set("Scatter\nPlot", $l + 4 * $w + 3 * $wp, $r, $wp, $h);
$t->Stroke($g->img, $scale);
