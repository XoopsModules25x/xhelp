...
require_once \dirname(__DIR__) . '/jpgraph_gantt.php';
...
for ($i = 0; $i < $n; ++$i) {
    if (0 === $i || 3 === $i) {
        ...
        $bar = new GanttBar($data[$i][0], [$data[$i][1], $data[$i][2], $data[$i][3]], $data[$i][2], $data[$i][3], '', 0.35);
        ...
        $bar = new GanttBar($data[$i][0], [$data[$i][1], $data[$i][2], $data[$i][3]], $data[$i][2], $data[$i][3], '', 0.45);
