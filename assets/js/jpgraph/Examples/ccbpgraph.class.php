...
class CCBPGraph
{
     ...
     */
    public function SetTitle($aTitle, $aSubTitle): void
    {
        $this->iTitle    = $aTitle;
     ...
     * @param int $aMax
     */
    public function SetXMinMax($aMin, $aMax): void
    {
     ...
     * @param int $aMap
     */
    public function SetColorMap($aMap): void
    {
     ...
     * @param int $aMax
     */
    public function SetYMinMax($aMin, $aMax): void
    {
    ...
    public function SetColorIndication(array $aSpec, array $aColors = null): void
    {
        ...
        if (null !== $aColors) {
            if (is_array($aColors) && 4 == count($aColors)) {
    ...
    private function Init(): void
    {
        ...
        $ytitle = new Text(self::YTitle, floor($lm * .75), ($height - $tm - $bm) / 2 + $tm);
        #$ytitle->SetFont(FF_VERA,FS_BOLD,$labelsize+1);
        $ytitle->SetAlign('right', 'center');
        $ytitle->SetAngle(90);
        $graph->add($ytitle);

        $xtitle = new Text(self::XTitle, ($width - $lm - $rm) / 2 + $lm, $height - 10);
        ...
        $cb = [];
        for ($i = 0; $i < 4; ++$i) {
            $cb[$i] = new LinePlot([$colarea[$i][0][1], $colarea[$i][1][1]], [$colarea[$i][0][0], $colarea[$i][1][0]]);
