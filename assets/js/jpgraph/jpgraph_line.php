...
class LinePlot extends Plot
{
    ...
    public function SetFilled($aFlg = true): void
    {
        $this->filled = $aFlg;
    }

    public function SetBarCenter($aFlag = true): void
    {
    ...
    public function SetStepStyle($aFlag = true): void
    {
    ...
    public function SetFillFromYMin($f = true): void
    {
        $this->fillFromMin = $f;
    }

    public function SetFillFromYMax($f = true): void
    {
        $this->fillFromMax = $f;
    }

    public function SetFillColor($aColor, $aFilled = true): void
    {
        //$this->color = $aColor;
    ...
    public function SetFillGradient($aFromColor, $aToColor, $aNumColors = 100, $aFilled = true): void
    {
    ...
    public function Legend($graph): void
    {
        if ('' != $this->legend) {
            if ($this->filled && !$this->fillgrad) {
                $graph->legend->Add($this->legend, $this->fill_color, $this->mark, 0, $this->legendcsimtarget, $this->legendcsimalt, $this->legendcsimwintarget);
            } elseif ($this->fillgrad) {
                $color = [$this->fillgrad_fromcolor, $this->fillgrad_tocolor];
                // In order to differentiate between gradients and cooors specified as an RGB triple
                $graph->legend->Add($this->legend, $color, '', -2 /* -GRAD_HOR */, $this->legendcsimtarget, $this->legendcsimalt, $this->legendcsimwintarget);
            } else {
                $graph->legend->Add($this->legend, $this->color, $this->mark, $this->line_style, $this->legendcsimtarget, $this->legendcsimalt, $this->legendcsimwintarget);
            }
        }
    }

    public function AddArea($aMin = 0, $aMax = 0, $aFilled = LP_AREA_NOT_FILLED, $aColor = 'gray9', $aBorder = LP_AREA_BORDER): void
    {
        if ($aMin > $aMax) {
            // swap
            $tmp  = $aMin;
    ...
    public function PreStrokeAdjust($graph): void
    {
    ...
    public function SetFastStroke($aFlg = true): void
    {
        $this->iFastStroke = $aFlg;
    }

    public function FastStroke($img, $xscale, $yscale, $aStartPoint = 0, $exist_x = true): void
    {
...
class AccLinePlot extends Plot
{
    ...
    public function SetInterpolateMode($aIntMode): void
    {
