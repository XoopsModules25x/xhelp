...
require_once __DIR__ . '/jpgraph_iconplot.php';
require_once __DIR__ . '/jpgraph_plotmark.inc.php';
...
class GanttActivityInfo
{
    ...
    public function Hide($aF = true): void
    {
        $this->iShow = !$aF;
    }

    public function Show($aF = true): void
    {
        $this->iShow = $aF;
    }

    // Specify font
    public function SetFont($aFFamily, $aFStyle = FS_NORMAL, $aFSize = 10): void
    {
    ...
    public function SetColumnMargin($aLeft, $aRight): void
    {
        $this->iLeftColMargin  = $aLeft;
        $this->iRightColMargin = $aRight;
    }

    public function SetFontColor($aFontColor): void
    {
    ...
    public function SetBackgroundColor($aColor): void
    {
        $this->iBackgroundColor = $aColor;
    }

    public function SetColTitles($aTitles, $aWidth = null): void
    {
        $this->iTitles = $aTitles;
        $this->iWidth  = $aWidth;
    }

    public function SetMinColWidth($aWidths): void
    {
    ...
    public function GetColStart($aImg, &$aStart, $aAddLeftMargin = false): void
    {
        $n      = count($this->iTitles);
    ...
    public function SetHeaderAlign($aAlign): void
    {
        $this->iHeaderAlign = $aAlign;
    }

    public function Stroke($aImg, $aXLeft, $aYTop, $aXRight, $aYBottom, $aUseTextHeight = false): void
    {
        if (!$this->iShow) {
            return;
        }
...
class GanttGraph extends Graph
{
    ...
    public function SetSimpleFont($aFont, $aSize): void
    {
        $this->iSimpleFont     = $aFont;
        $this->iSimpleFontSize = $aSize;
    }

    public function SetSimpleStyle($aBand, $aColor, $aBkgColor): void
    {
    ...
    public function CreateSimple($data, $constrains = [], $progress = []): void
    {
        $num = count($data);
        for ($i = 0; $i < $num; ++$i) {
            switch ($data[$i][1]) {
                ...
                case ACTYPE_NORMAL:
                    ...
                    $n = count($constrains);
                    for ($j = 0; $j < $n; ++$j) {
                        if (empty($constrains[$j]) || (3 != count($constrains[$j]))) {
                    ...
                    $n = count($progress);
                    for ($j = 0; $j < $n; ++$j) {
                        if (empty($progress[$j]) || (2 != count($progress[$j]))) {
    ...
    public function SetZoomFactor($aZoom): void
    {
        $this->iZoomFactor = $aZoom;
    }

    // Set what headers should be shown
    public function ShowHeaders($aFlg): void
    {
    ...
    // as vertical margin
    public function SetLabelVMarginFactor($aVal): void
    {
        $this->iLabelVMarginFactor = $aVal;
    }

    // Synonym to the method above
    public function SetVMarginFactor($aVal): void
    {
        $this->iLabelVMarginFactor = $aVal;
    }

    // Add a new Gantt object
    public function Add($aObject): void
    {
    ...
    public function GetBarMinMax()
    {
        $start = 0;
        $n     = count($this->iObj);
        while ($start < $n && false === $this->iObj[$start]->GetMaxDate()) {
            ++$start;
        }
        if ($start >= $n) {
    ...
    public function AutoSize(): void
    {
        if (null === $this->img->img) {
            ...
            if ($this->scale->IsDisplayWeek()) {
                ...
                if (WEEKSTYLE_FIRSTDAY2WNBR == $this->scale->week->iStyle) {
                    $fsw += 8;
                } elseif (WEEKSTYLE_FIRSTDAYWNBR == $this->scale->week->iStyle) {
                    $fsw += 7;
                } else {
                    $fsw += 4;
                }
            ...
            if (!$this->scale->IsDisplayDay() && !$this->scale->IsDisplayHour()
                && !((WEEKSTYLE_FIRSTDAYWNBR == $this->scale->week->iStyle
                      || WEEKSTYLE_FIRSTDAY2WNBR == $this->scale->week->iStyle)
                     && $this->scale->IsDisplayWeek())) {
    ...
    public function StrokeConstrains(): void
    {
        $n = count($this->iObj);
...
class PredefIcons
{
    ...
    public function GetImg($aIdx)
    {
        ...
        return Image::CreateFromString(base64_decode($this->iBuiltinIcon[$aIdx][1], true));
    }
    ...
    public function __construct()
    {
        ...
        $this->iBuiltinIcon[0][1] = 'iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsSAAALEgHS3X78AAAA'
        ...
        $this->iBuiltinIcon[1][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAFgAWABY9j+ZuwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[2][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAP8A/wD/oL2nkwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[3][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAP8A/wD/oL2nkwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[4][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAP8A/wD/oL2nkwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[5][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAA4AIwBbgMF12wAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[6][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAP8A/wD/oL2nkwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[7][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAAAAAAAA+UO7fwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[8][1] = 'iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAAAAAAAA+UO7fwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[9][1] = 'iVBORw0KGgoAAAANSUhEUgAAACgAAAAoCAMAAAC7IEhfAAAAaVBMVEX//////2Xy8mLl5V/Z2VvMzFi/v1WyslKlpU+ZmUyMjEh/'
        ...
        $this->iBuiltinIcon[10][1] = 'iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAABGdBTUEAALGPC/xhBQAAAAZiS0dEANAAtwClFht71AAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[11][1] = 'iVBORw0KGgoAAAANSUhEUgAAACIAAAAiCAYAAAA6RwvCAAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAAAAAAAA+UO7fwAAAAlwSFlz'
        ...
        $this->iBuiltinIcon[12][1] = 'iVBORw0KGgoAAAANSUhEUgAAACIAAAAiCAYAAAA6RwvCAAAABGdBTUEAALGPC/xhBQAAAAZiS0dEAAAAAAAA+UO7fwAAAAlwSFlz'
...
class IconImage
{
    ...
    public function SetAlign($aX = 'left', $aY = 'center'): void
    {
        $this->ixalign = $aX;
        $this->iyalign = $aY;
    }

    public function Stroke($aImg, $x, $y): void
    {
        if ('right' == $this->ixalign) {
            $x -= $this->iWidth;
        } elseif ('center' == $this->ixalign) {
            $x -= round($this->iWidth / 2 * $this->iScale);
        }

        if ('bottom' == $this->iyalign) {
            $y -= $this->iHeight;
        } elseif ('center' == $this->iyalign) {
            $y -= round($this->iHeight / 2 * $this->iScale);
        }

        $aImg->Copy($this->iGDImage, $x, $y, 0, 0, round($this->iWidth * $this->iScale), round($this->iHeight * $this->iScale), $this->iWidth, $this->iHeight);
    }
}
...
class TextProperty
{
    ...
    //---------------
    // PUBLIC METHODS
    public function Set($aTxt): void
    {
        $this->iText = $aTxt;
    }

    public function SetCSIMTarget($aTarget, $aAltText = '', $aWinTarget = ''): void
    {
    ...
    public function SetCSIMAlt($aAltText): void
    {
        if (is_string($aAltText)) {
            $aAltText = [$aAltText];
        }
    ...
    // Set alignment
    public function Align($aHAlign, $aVAlign = 'bottom'): void
    {
    ...
    // Synonym
    public function SetAlign($aHAlign, $aVAlign = 'bottom'): void
    {
        $this->iHAlign = $aHAlign;
        $this->iVAlign = $aVAlign;
    }
    ...
    public function SetColumnFonts($aFontArray): void
    {
        if (!is_array($aFontArray) || 3 != count($aFontArray[0])) {
    ...
    public function Stroke($aImg, $aX, $aY): void
    {
        if ($this->iShow) {
            $aImg->SetColor($this->iColor);
            $aImg->SetFont($this->iFFamily, $this->iFStyle, $this->iFSize);
...
class HeaderProperty
{
    ...
    public function SetIntervall($aInt): void
    {
        $this->iIntervall = $aInt;
    }

    public function SetInterval($aInt): void
    {
        $this->iIntervall = $aInt;
    }
    ...
    public function SetFontColor($aColor): void
    {
    ...
    public function SetFrameWeight($aWeight): void
    {
        $this->iFrameWeight = $aWeight;
    }

    public function SetFrameColor($aColor): void
    {
        $this->iFrameColor = $aColor;
    }

    // Only used by day scale
    public function SetWeekendColor($aColor): void
    {
        $this->iWeekendBackgroundColor = $aColor;
    }

    // Only used by day scale
    public function SetSundayFontColor($aColor): void
    {
        $this->iSundayTextColor = $aColor;
    }

    public function SetTitleVertMargin($aMargin): void
    {
        $this->iTitleVertMargin = $aMargin;
    }

    public function SetLabelFormatString($aStr): void
    {
        $this->iLabelFormStr = $aStr;
    }

    public function SetFormatString($aStr): void
    {
...
class GanttScale
{
    ...
    public function UseWeekendBackground($aShow): void
    {
    ...
    public function SetVertLayout($aLayout): void
    {
        $this->iVertLayout = $aLayout;
    }

    // Which locale should be used?
    public function SetDateLocale($aLocale): void
    {
    ...
    // widest title)
    public function SetLabelWidth($aLabelWidth): void
    {
    ...
    public function SetWeekStart($aStartDay): void
    {
    ...
    public function SetVertSpacing($aSpacing): void
    {
    ...
    public function SetRange($aMin, $aMax): void
    {
    ...
    public function AdjustStartEndDay(): void
    {
    ...
    public function SetTableTitleBackground($aColor): void
    {
    ...
    public function TranslateVertPos($aPos, $atTop = false)
    {
        ...
        if (GANTT_EVEN == $this->iVertLayout) {
    ...
    public function StrokeMinutes($aYCoord, $getHeight = false)
    {
        ...
        if ($this->minute->iShowLabels) {
            ...
            $yb = $yt + $img->GetFontHeight() + $this->minute->iTitleVertMargin + $this->minute->iFrameWeight;
            if ($getHeight) {
    ...
    public function StrokeHours($aYCoord, $getHeight = false)
    {
        ...
        if ($this->hour->iShowLabels) {
            ...
            $yb = $yt + $img->GetFontHeight() + $this->hour->iTitleVertMargin + $this->hour->iFrameWeight;
            if ($getHeight) {
                return $yb - $img->top_margin;
            }
            $xb = $img->width - $img->right_margin + 1;
            ...
            for ($j = 0; $j < $this->GetNumberOfDays(); ++$j, $day += 1, $day %= 7) {
                for ($i = 0; $i < $n; ++$i, $x += $width) {
                    ...
                    //$datestamp += $minint*60
                    $datestamp = mktime(date('H', $datestamp), date('i', $datestamp) + $minint, 0, date('m', $datestamp), date('d', $datestamp) + 1, date('Y', $datestamp));
                }
            }
    ...
    public function StrokeDays($aYCoord, $getHeight = false)
    {
        ...
        if ($this->day->iShowLabels) {
            ...
            for ($i = 0; $i < $this->GetNumberOfDays(); ++$i, $x += $daywidth, $day += 1, $day %= 7) {
                ...
                $img->StrokeText(round($x + $daywidth / 2 + 1), round($yb - $this->day->iTitleVertMargin), $txt);
    ...
    public function StrokeWeeks($aYCoord, $getHeight = false)
    {
        if ($this->week->iShowLabels) {
            ...
            $x = $xt;
            if (WEEKSTYLE_WNBR == $this->week->iStyle) {
                $img->SetTextAlign('center');
                $txtOffset = $weekwidth / 2 + 1;
            } elseif (WEEKSTYLE_FIRSTDAY == $this->week->iStyle
                      || WEEKSTYLE_FIRSTDAY2 == $this->week->iStyle
                      || WEEKSTYLE_FIRSTDAYWNBR == $this->week->iStyle
                      || WEEKSTYLE_FIRSTDAY2WNBR == $this->week->iStyle) {
            ...
            for ($i = 0; $i < $this->GetNumberOfDays() / 7; ++$i, $x += $weekwidth) {
                ...
                if (WEEKSTYLE_WNBR == $this->week->iStyle) {
                    $txt = sprintf($this->week->iLabelFormStr, $weeknbr);
                          ...
                          || WEEKSTYLE_FIRSTDAYWNBR == $this->week->iStyle) {
                    $txt = date('j/n', $week);
                } elseif (WEEKSTYLE_FIRSTDAY2 == $this->week->iStyle
                          || WEEKSTYLE_FIRSTDAY2WNBR == $this->week->iStyle) {
                ...
                if (WEEKSTYLE_FIRSTDAYWNBR == $this->week->iStyle
                    || WEEKSTYLE_FIRSTDAY2WNBR == $this->week->iStyle) {
    ...
    public function StrokeMonths($aYCoord, $getHeight = false)
    {
        if ($this->month->iShowLabels) {
            ...
            if ($monthwidth >= 1.2 * $img->GetTextWidth($monthName)) {
                $img->SetColor($this->month->iTextColor);
                $img->StrokeText(round($xt + $monthwidth / 2 + 1), round($yb - $this->month->iTitleVertMargin), $monthName);
            }
            $x = $xt + $monthwidth;
            while ($x < $xb) {
                ...
                if ($w >= 1.2 * $img->GetTextWidth($monthName)) {
                    $img->SetColor($this->month->iTextColor);
                    $img->StrokeText(round($x + $w / 2 + 1), round($yb - $this->month->iTitleVertMargin), $monthName);
                }
                $x += $monthwidth;
            }
    ...
    public function StrokeYears($aYCoord, $getHeight = false)
    {
        if ($this->year->iShowLabels) {
            ...
            if ($yearwidth >= 1.20 * $img->GetTextWidth('' . $year)) {
                ...
                $img->StrokeText(round($xt + $yearwidth / 2 + 1), round($yb - $this->year->iTitleVertMargin), $year);
            }
            $x = $xt + $yearwidth;
            while ($x < $xb) {
                ...
                if ($w >= 1.2 * $img->GetTextWidth('' . $year)) {
                    $img->SetColor($this->year->iTextColor);
                    $img->StrokeText(round($x + $w / 2 + 1), round($yb - $this->year->iTitleVertMargin), $year);
                }
                $x += $yearwidth;
            }
    ...
    public function StrokeTableHeaders($aYBottom): void
    {
        $img = $this->iImg;
        $xt  = $img->left_margin;
        $yt  = $img->top_margin;
        ...
        // to the left
        $fancy = 'fancy' == $this->divider->iStyle;
        if ($fancy) {
...
class GanttPlotObject
{
    ...
    public function SetCSIMTarget($aTarget, $aAlt = '', $aWinTarget = ''): void
    {
        if (!is_string($aTarget)) {
    ...
    public function SetCSIMAlt($aAlt): void
    {
        if (!is_string($aAlt)) {
            $tv = mb_substr(var_export($aAlt, true), 0, 40);
            JpGraphError::raiseL(6025, $tv);
    ...
    public function SetConstrain($aRow, $aType, $aColor = 'black', $aArrowSize = ARROW_S2, $aArrowType = ARROWT_SOLID): void
    {
        $this->constraints[] = new GanttConstraint($aRow, $aType, $aColor, $aArrowSize, $aArrowType);
    }

    public function SetConstrainPos($xt, $yt, $xb, $yb): void
    {
    ...
    public function SetCaptionMargin($aMarg): void
    {
    ...
    public function SetLabelLeftMargin($aOff): void
    {
        $this->iLabelLeftMargin = $aOff;
    }

    public function StrokeActInfo($aImg, $aScale, $aYPos): void
    {
        $cols = [];
...
class Progress
{
    ...
    public function Set($aProg): void
    {
    ...
    public function SetPattern($aPattern, $aColor = 'blue', $aDensity = 98): void
    {
    ...
    public function SetFillColor($aColor): void
    {
        $this->iFillColor = $aColor;
    }

    public function SetHeight($aHeight): void
    {
...
class HorizontalGridLine
{
    ...
    public function SetRowFillColor($aColor1, $aColor2 = ''): void
    {
        $this->iRowColor1 = $aColor1;
        $this->iRowColor2 = $aColor2;
    }

    public function SetStart($aStart): void
    {
        $this->iStart = $aStart;
    }

    public function Stroke($aImg, $aScale): void
    {
        if (!$this->iShow) {
            return;
        }
...
class GanttBar extends GanttPlotObject
{
    ...
    //---------------
    // PUBLIC METHODS
    public function SetShadow($aShadow = true, $aColor = 'gray'): void
    {
        $this->iShadow      = $aShadow;
        $this->iShadowColor = $aColor;
    }

    public function SetBreakStyle($aFlg = true, $aLineStyle = 'dotted', $aLineWeight = 1): void
    {
    ...
    public function SetPattern($aPattern, $aColor = 'blue', $aDensity = 95): void
    {
        $this->iPattern        = $aPattern;
    ...
    public function Stroke($aImg, $aScale): void
    {
        ...
        if ($limst == $st) {
            ...
            // bar.
            if (MARK_LEFTTRIANGLE == $this->leftMark->GetType()) {
                $y = $yb;
            }
        ...
        if ($limen == $en) {
            ...
            // special so that these marks are placed right under the
            // bar.
            if (MARK_RIGHTTRIANGLE == $this->rightMark->GetType()) {
                $y = $yb;
            }
...
class GanttVLine extends GanttPlotObject
{
    ...
    // Gantt chart is used
    public function SetRowSpan($aStart, $aEnd = -1): void
    {
        $this->iStartRow = $aStart;
        $this->iEndRow   = $aEnd;
    }

    public function SetDayOffset($aOff = 0.5): void
    {
    ...
    public function SetTitleMargin($aMarg): void
    {
...
class LinkArrow
{
    ...
    public function SetSize($aSize): void
    {
        $this->iSize = $aSize;
    }

    public function SetType($aType): void
    {
        $this->iType = $aType;
    }
...
class GanttLink
{
    ...
    public function SetPos($x1, $y1, $x2, $y2): void
    {
        $this->ix1 = $x1;
        $this->ix2 = $x2;
        $this->iy1 = $y1;
    ...
    public function SetPath($aPath): void
    {
    ...
    public function SetArrow($aSize, $aType = ARROWT_SOLID): void
    {
