...
// Gnome Vera font
// Available from https://www.gnome.org/fonts/
define('FF_VERA', 18);
...
// Extra fonts
// Download fonts from
// https://www.webfontlist.com
// https://www.webpagepublicity.com/free-fonts.html
// https://www.fontonic.com/fonts.asp?width=d&offset=120
// https://www.fontspace.com/category/famous
...
// See https://mix-mplus-ipa.sourceforge.jp/
...
class LanguageConv
{
    ...
    public function Convert($aTxt, $aFF)
    {
        if (LANGUAGE_GREEK) {
            ...
            if (CYRILLIC_FROM_WINDOWS && (!defined('LANGUAGE_CHARSET') || mb_stristr(LANGUAGE_CHARSET, 'windows-1251'))) {
                $aTxt = convert_cyr_string($aTxt, 'w', 'k');
            }
            if (!defined('LANGUAGE_CHARSET') || mb_stristr(LANGUAGE_CHARSET, 'koi8-r') || mb_stristr(LANGUAGE_CHARSET, 'windows-1251')) {
            ...
            return iconv('BIG5', 'UTF-8', $aTxt);
                  ...
                  && (FF_MINCHO == $aFF || FF_GOTHIC == $aFF || FF_PMINCHO == $aFF || FF_PGOTHIC == $aFF)) {
            if (!function_exists('mb_convert_encoding')) {
                JpGraphError::raiseL(25127);
            }

            return mb_convert_encoding($aTxt, 'UTF-8', 'EUC-JP');
        } elseif (FF_DAVID == $aFF || FF_MIRIAM == $aFF || FF_AHRON == $aFF) {
    ...
    public static function gr_iso2uni($isoline)
    {
        ...
        for ($i = 0, $iMax = mb_strlen($isoline); $i < $iMax; ++$i) {
            $thischar = mb_substr($isoline, $i, 1);
            $charcode = ord($thischar);
            $uniline  .= ($charcode > 179 && 183 != $charcode && 187 != $charcode && 189 != $charcode) ? '&#' . (900 + ($charcode - 180)) . ';' : $thischar;
        }
    ...
    {
        $uniline = '';
        for ($i = 0, $iMax = mb_strlen($winline); $i < $iMax; ++$i) {
            $thischar = mb_substr($winline, $i, 1);
            $charcode = ord($thischar);
            if (161 == $charcode || 162 == $charcode) {
                $uniline .= '&#' . (740 + $charcode) . ';';
            } else {
                $uniline .= (($charcode > 183 && 187 != $charcode && 189 != $charcode) || 180 == $charcode) ? '&#' . (900 + ($charcode - 180)) . ';' : $thischar;
            }
        }
    ...
    public function File($family, $style = FS_NORMAL)
    {
        ...
        if (!$fam) {
            JpGraphError::raiseL(
                25046,
                $family
            ); //("Specified TTF font family (id=$family) is unknown or does not exist. Please note that TTF fonts are not distributed with JpGraph for copyright reasons. You can find the MS TTF WEB-fonts (arial, courier etc) for download at https://corefonts.sourceforge.net/");
        }
        $ff = @$fam[$style];
        ...
        $jpgraph_font_dir = __DIR__ . '/fonts/';
        ...
        foreach ($ff as $font_file) {
            ...
            if (true === file_exists($jpgraph_font_file) && true === is_readable($jpgraph_font_file)) {
            ...
            if (true === file_exists($font_file) && true === is_readable($font_file)) {
                break;
            }
        }
...
class SymChar
{
    public static function Get($aSymb, $aCapital = false)
    {
        ...
        $i     = 0;
        $found = false;
        $aSymb = \mb_strtolower($aSymb);
        while ($i < $n && !$found) {
            $found = $aSymb === $iSymbols[$i++][0];
        }
        if ($found) {
            $ca = $iSymbols[--$i];
            if ($aCapital && 3 == count($ca)) {
                $s = $ca[2];
            } else {
                $s = $ca[1];
            }
