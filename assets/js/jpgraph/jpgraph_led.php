...
class jpgraph_led
{
    ...
    public function SetSupersampling($aSuperSampling = 2): void
    {
    ...
    public function Stroke($aValStr, $aColor = 0, $aFileName = ''): void
    {
        $this->StrokeNumber($aValStr, $aColor, $aFileName);
    }

    public function StrokeNumber($aValStr, $aColor = 0, $aFileName = ''): void
    {
        if ($aColor < 0 || $aColor >= count($this->iColorSchema)) {
            $aColor = 0;
        }

        if (0 == ($n = mb_strlen($aValStr, 'utf8'))) {
            $aValStr = ' ';
            $n       = 1;
        }
