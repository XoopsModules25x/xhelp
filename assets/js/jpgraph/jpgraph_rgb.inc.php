...
class RGB
{
    ...
    public function Color($aColor)
    {
        if (is_string($aColor)) {
            ...
            if (empty($matches[5])) {
                $r = 1 == mb_strlen($matches[2]) ? $matches[2] . $matches[2] : $matches[2];
                $g = 1 == mb_strlen($matches[3]) ? $matches[3] . $matches[3] : $matches[3];
                $b = 1 == mb_strlen($matches[4]) ? $matches[4] . $matches[4] : $matches[4];
                $r = hexdec($r);
                $g = hexdec($g);
                $b = hexdec($b);
            } else {
