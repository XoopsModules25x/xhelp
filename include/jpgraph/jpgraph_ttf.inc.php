...
class LanguageConv
{
    ...
    public function Convert($aTxt, $aFF)
    {
        if (LANGUAGE_GREEK) {
            ...
            if (!defined('LANGUAGE_CHARSET') || stristr(LANGUAGE_CHARSET, 'koi8-r')
                || stristr(LANGUAGE_CHARSET, 'windows-1251')) {
            ...
            if ($this->g2312 === null) {
                include_once __DIR__ . '/jpgraph_gb2312.php';
    ...
    {
        $uniline = '';
        for ($i = 0, $iMax = strlen($isoline); $i < $iMax; ++$i) {
    ...
    {
        $uniline = '';
        for ($i = 0, $iMax = strlen($winline); $i < $iMax; ++$i) {
...
class TTF
{
    ...
    {
        $this->font_files[FF_USERFONT] = array(
            FS_NORMAL     => $aNormal,
            FS_BOLD       => $aBold,
            FS_ITALIC     => $aItalic,
    ...
    {
        $this->font_files[FF_USERFONT1] = array(
            FS_NORMAL     => $aNormal,
            FS_BOLD       => $aBold,
            FS_ITALIC     => $aItalic,
            FS_BOLDITALIC => $aBoldIt
    ...
    {
        $this->font_files[FF_USERFONT2] = array(
            FS_NORMAL     => $aNormal,
            FS_BOLD       => $aBold,
            FS_ITALIC     => $aItalic,
            FS_BOLDITALIC => $aBoldIt
    ...
    {
        $this->font_files[FF_USERFONT3] = array(
            FS_NORMAL     => $aNormal,
            FS_BOLD       => $aBold,
            FS_ITALIC     => $aItalic,
            FS_BOLDITALIC => $aBoldIt
